...
final class NativeRuntime {
    ...
    private static void emitSecurityFunctions(StringBuilder sb) {
        sb.append("""
            ...
            .Lstr_jwt_iat: .byte 0x22, 0x69, 0x61, 0x74, 0x22, 0x3a
            .Lstr_jwt_expk: .byte 0x2c, 0x22, 0x65, 0x78, 0x70, 0x22, 0x3a
            ...
            kof_sec_jwt_create_ttl:
                ...
                movq %rsi, %r12          # secret
                movl %edx, %r13d         # ttl
                subq $1024, %rsp         # payload 0, hdrb64 300, payloadb64 340, sig 740
                call kof_now
                movq $1000, %rcx
                xorq %rdx, %rdx
                divq %rcx
                movq %rax, %r14          # now_s
                # achar o último '}' no claims
                movl 16(%rbx), %eax
                leaq 24(%rbx), %r8
            ...
            .Ljw_brace_done:
                ...
                jne .Ljw_bad_claims
            .Ljw_head:
                # payload = head(0..lastBrace) + sep + "\"iat\":N,\"exp\":M}"
            ...
            .Ljw_exp_done:
                movb $125, (%rsp,%r15)   # '}'
                incq %r15
                # header b64url: "{\"alg\":\"HS256\",\"typ\":\"JWT\"}" (29 bytes)
                leaq 300(%rsp), %rdi
                leaq .Lstr_jwt_header(%rip), %rsi
                movq $29, %rdx
                call kof_b64url_encode_internal
                movq %rax, %r13          # header b64 len (39)
                # payload b64url
                leaq 340(%rsp), %rdi
                movq %rsp, %rsi
                movq %r15, %rdx
                ...
                # sig = sign_buf(secret, hdrb64(300), hlen, payloadb64(340), plen)
                movq %r12, %rdi
                leaq 300(%rsp), %rsi
                movq %r13, %rdx
                leaq 340(%rsp), %rcx
                ...
                # token total = hlen + 1 + plen + 1 + siglen
                movl %r13d, %eax
                addl %r14d, %eax
                addl 16(%r15), %eax
                addl $2, %eax
                ...
                xorq %rcx, %rcx
                xorq %r8, %r8            # out len
                movl %r13d, %r9d         # header b64 len
                movl %r14d, %r10d        # payload b64 len
            .Ljw_th:
                cmpl %r9d, %ecx
                jge .Ljw_td1
            ...
            .Ljv_dots_done:
                ...
                cmpq $-1, %r15
                je .Ljv_bad_token
                # 2. header: b64url decode [data, data+d1) → 0(%rsp)
                movq %rsp, %rdi
                movq %r8, %rsi
                movq %r15, %rdx
            ...
            .Ljv_alg_next:
                ...
                jmp .Ljv_alg
            .Ljv_alg_ok:
                # 3. sig esperada: sign_buf(secret, data, d1, data+d1+1, d2-d1-1)
                movq %r12, %rdi
                movq 1520(%rsp), %r8
                movq %r8, %rsi
                movq %r15, %rdx
                ...
                # 4. constant-time: compare part2 com o expected
                movq 1528(%rsp), %rax
                subq %r10, %rax
                decq %rax                # part2 len
                movq %rax, %r9
            ...
            .Ljv_sig_done:
                testq %r11, %r11
                jnz .Ljv_bad_sig
                # 5. payload: decode part1 → 160(%rsp), montar KofString
                movq 1520(%rsp), %r8
                leaq 160(%rsp), %rdi
                leaq 1(%r15,%r8), %rsi   # data + d1 + 1
                movq %r10, %rdx
                subq %r15, %rdx
                decq %rdx                # d2 - d1 - 1
                call kof_b64url_decode_internal
                movq %rax, %r9           # payload len
                leaq 160(%rsp), %rdi
                movq %rax, %rsi
                call .Ljf_mkstr
