// vkchain64.c — M36: mesma cadeia do vkchain.c com elementos int64.
// Contrato:
//   int  vkchain64_init(const char* spv_path)         → 0 ok / !=0 erro
//   const char* vkchain64_fail_reason(void)
//   int  vkchain64_dispatch(int64_t* a, int64_t* b, int64_t* c, int m, int n, int k)
//        → 0 ok / !=0 (caller usa golden CPU)
// matmul64.comp: 3 SSBOs ivec2 (pack64) + push (M,N,K), local_size 16x16.
// Buffers: 256 elems × 8B = 2KB cada (m*k/k*n/m*n ≤ 256 elems).
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int vkchain64_init(const char* spv_path){
    ...
    for (int i=0;i<3;i++){
        VkBufferCreateInfo bci={VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,0,0,256*8,VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,VK_SHARING_MODE_EXCLUSIVE,0,0};
        ...
        CK(vkMapMemory(dev,mems[i],0,256*8,0,&maps[i]), "map");
    }
    ...
    for (int i=0;i<3;i++){
        dbi[i]=(VkDescriptorBufferInfo){bufs[i],0,256*8};
...
int vkchain64_dispatch(int64_t* a, int64_t* b, int64_t* c, int m, int n, int k){
    if (!inited) return -1;
    if ((size_t)m*k > 256 || (size_t)k*n > 256 || (size_t)m*n > 256) return -2; // >256 elems: não cabe
    memcpy(maps[0], a, (size_t)m*k*8);
    memcpy(maps[1], b, (size_t)k*n*8);
    memset(maps[2], 0, (size_t)m*n*8);
