...
| **MySQL Native prepared** — COM_STMT_PREPARE/EXECUTE binário | `EM CURSO` | agente-nativo-val | main | `NativeDbRuntime.java` (novo, ≤500 linhas), `KofDb.java`, `CompilerDriver.java`, `NativeRuntime.java` | wire handshake/COM_QUERY OK, falta binário PREPARE/EXECUTE (01/09 revertido). Segue padrão HTTP002 (módulo novo, sem violar 500 linhas). Fetch 03/09: idiomatic-fixes em `c664463` (docs), planning-future `e525993` (DOING.md) — sem conflito de código |

## Concluídos recentemente

| Gap/Item | Estado | Dono | Data | Prova |
|---|---|---|---|---|
| **NATIVE002** — validação 13/13 + observability real (riscv64/aarch64) | `FEITO` | agente-nativo-val | 03/09 | `b20aa49` — `kof_validation_*` 13/13 + `kof_observability_*` counter/gauge/histogram/metrics/health/readiness/liveness/trace/span em `RISCV_RUNTIME_ASM` + `translateRiscvToAarch64`; `qemu-riscv64`/`qemu-aarch64` `validation`/`observability` OK (step + assert) |
