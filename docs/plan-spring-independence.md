...
**Versão:** 0.2.6-beta (805 testes; `kof.http` JVM+JS com retry/circuit; web ws/sse JVM; free-list + pthread spawn Native)
...
- [x] Testes E2E (subprocesso + sockets reais) verdes; `mvn test` verde
      (805 em 0.2.6-beta; 459/459 na época incluindo os 9 de `KofWebE2ETest`; ver `docs/status.md:10-28`).
...
paralela (`defaultParameters` no target JS). **Atual 0.2.6-beta (27/08): `mvn test` 805 (788+8+5+4), golden 16/16, integration 9/9** — ver `docs/status.md:10-28`.