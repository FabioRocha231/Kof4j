...
Estado atual: ✅ estável (JVM V21, ASM, virtual threads, 805 testes 02/09;
...
- releases por 2 jobs (`release.yml`: `test-and-bump` exporta `bump_sha` → `package-and-release` checkeia o commit de bump + sanity check de versão) por push na `main`, por plataforma linux-x86_64 / macos-arm64 / windows-x86_64 (testes 805 → bump → package 3 plataformas → GitHub Release);
