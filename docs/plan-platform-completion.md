...
| Build | `mvn clean package` PASS, `mvn test` 741 (728+8+5), `VERSION` 0.2.6-beta |
...
| ✅ 0.2.6-beta GC + MySQL + Bugs | free-list `kof_free_head` (reuso mmap; mark-sweep pendente), MySQL wire protocol WIP (scramble SHA-1 + `user:pass@`), large-project `import a.b.C` (`CompilerDriver.java:243`), `List.get`/`listOf`, SIGPIPE Windows | `mvn test` 741, `VERSION` 0.2.6-beta |
