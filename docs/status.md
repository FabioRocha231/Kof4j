...
7. ✅ `WebSocket` `app.ws("/chat") { }` + `SSE` `sse.send/event/close` — ✅ JVM (30/08; PRs 14-17: persistent-conn/route-kinds, SSE, handshake RFC 6455, frame codec+máscara; hardening 01/09: RSV/opcodes/control/UTF-8/fragmentação/close + SSE CRLF/event-name; `KofWebSseE2ETest 10/10` `KofWebWsE2ETest 28/28` `KofWsFrameTest 7/7`)
