...
> **Build:** `mvn clean package` PASS, `mvn test` 805 (788 kof-compiler +8 kof-script +5 kof-c-compiler +4 kof-cli), golden 16/16, integration 9/9, `scripts/package.sh` PASS, `VERSION` 0.2.6-beta, `release.yml` 2 jobs (`test-and-bump` → `package-and-release`) × 3 plataformas, Windows SIGPIPE fix.
...
## 2.6 Testes (805 JUnit: 788 kof-compiler +8 kof-script +5 kof-c-compiler +4 kof-cli) — por módulo (27/08)
...
Golden: `tests/golden/` 16/16 (8 casos × jvm+native). Integration: `tests/run-integration.sh` 9/9. `mvn test` 805.
