...
> Tabela reflete 0.2.6-beta (02/09) — Build `mvn test` **810** (793+8+5+4), golden 16/16, integration 9/9. DoD em `docs/plan-platform-completion.md`.
...
| Native riscv64/aarch64 codegen | `NATIVE002` | **riscv64 core completo (02/09)**: `NativeRiscv64E2ETest 13/13` — runtime + codegen em **asm puro** (raw syscalls, **sem C**; `as`+`ld` estático) + qemu: println, var, if/else, aritmética, **classes (virtual dispatch/fields), arrays, List, switch, try/catch/throw, pattern matching (switch String s/instanceof/as), String methods, recursão**. aarch64 ainda stub (portando o mesmo core). Paridade total x86 (JSON/DB/HTTP/concorrência/UI/net) pendente nos dois. **Ver `docs/native-multiarch.md`** |
