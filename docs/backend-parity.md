...
| `kof.db`/`kof.orm` | ✅ | ✅ (SQLite `.so` direto; **`transaction {}` commit/rollback 01/09** (EH asm + BEGIN/COMMIT/ROLLBACK); **MySQL wire protocol** — handshake+scramble+auth-switch+COM_QUERY+resultset 31/08)/ORM001 | DB001/ORM001 | MySQL native `nativeMysqlWireProtocol`; `nativeTransaction{Commits,RollsBack}`; prepared statements pendentes |
