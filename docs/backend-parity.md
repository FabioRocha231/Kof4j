...
| Native riscv64/aarch64 codegen | `NATIVE002` | **riscv64 parcial (02/09)**: `NativeRiscv64E2ETest 4/4` — `kof_main` em asm (stack machine) + runtime C (gcc cruzado) + qemu: println String/Int, var, if/else, aritmética/comparações Int. aarch64 ainda stub; coleções/classe/`instanceof`/`switch` cross pendentes. **Ver `docs/native-multiarch.md`** (estado real + como finalizar) |
