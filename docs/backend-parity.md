...
| GC mark-sweep no Native | ✅ | `kof_gc_sweep` real (flag bit1) + auto-collect (exaustão → collect antes do mmap) — `KofGcE2ETest 3/3` (03/09, agente-planning) |
| MySQL nativo completo | — | **wire protocol ✅** (handshake + scramble SHA-1 + auth-switch `mysql_native_password` + COM_QUERY + resultset — 31/08) + **prepared statements binários ✅ 03/09** (COM_STMT_PREPARE/EXECUTE + binary-rows, `NativeDbPrepared.java` + `KofDbE2ETest.nativeMysqlPreparedBinary`; binds `?` com aspas/injection intactos; fallback COM_QUERY se PREPARE falhar) |
