...
> Tabela reflete 0.2.6-beta (03/09) — Build `mvn test` **814** (797+8+5+4), golden 16/16, integration 9/9. DoD em `docs/plan-platform-completion.md`.
...
| kof.time (`now()`, `sleep`, `interval`) | ✅ | ✅ (now/sleep/**interval**) | ✅ (now/sleep/**interval**) | `interval`/`every` JVM (ScheduledExecutor) + **Native (01/09, `time.interval` reusa o scheduler — SCHED001; mutação por referência validada)** + **JS **TIME001 fechado 02/09** — fila cooperativa bombeada por `time.sleep` no GraalJS (sem event loop), `setInterval` nativo em browser/Node; `KofTimeE2ETest.jsIntervalRunsPeriodicallyUntilCancelled`** |
...
| MySQL nativo completo | — | **wire protocol ✅** (handshake + scramble SHA-1 + auth-switch `mysql_native_password` + COM_QUERY + resultset coldefs/rows/EOF — 31/08) + **prepared statements binários ✅ 03/09** (COM_STMT_PREPARE/EXECUTE + binary-rows com tipos, `NativeDbPrepared.java`, `KofDbE2ETest.nativeMysqlPreparedBinary`; binds `?` com aspas/injection intactos; fallback COM_QUERY substituição se PREPARE falhar) |
